...
def write_layer(gis_options, layer):
    ...
    if layer.type != 'RASTER':
        ...
        # MapServer 8.0 removed CLASS-level MAXSCALE in favor of MAXSCALEDENOM.
        if layer.max_scale:
            mapfile.write('      MAXSCALEDENOM %s\n' % (layer.max_scale))

        # MapServer 8.0 removed the CLASS-level styling shortcuts (COLOR, SYMBOL,
        # SIZE, ...); they must now live inside a STYLE block.
        mapfile.write('      STYLE\n')
...
def write_lineplot(gis_options, lineplot, shpfilename):
    ...
    mapfile.write('    CLASS\n')
    mapfile.write('      MAXSCALEDENOM 60000.00\n')
    mapfile.write('      STYLE\n')
...
def write_mapserver_footer(gis_options):
    ...
    mapfile.write('    CLASS\n')
    mapfile.write('      MINSCALEDENOM 60000\n')
    mapfile.write('      STYLE\n')
    ...
    mapfile.write('    STATUS ON\n')
    mapfile.write('    CLASS\n')
    mapfile.write('      MAXSCALEDENOM 60000\n')
    mapfile.write('      STYLE\n')
    mapfile.write('        COLOR 112 128 144\n')
    mapfile.write('      END\n')
    mapfile.write('      LABEL\n')
